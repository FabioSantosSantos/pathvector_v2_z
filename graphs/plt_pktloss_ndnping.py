...
for filename in args.ndvr:
    f = open(filename)
    result = json.load(f)
    first_time = float(sorted(result.keys())[0])
    for k in result:
        x = float(k) - first_time
        if x not in data_ndvr:
            data_ndvr[x] = []
        #data_ndvr[x].append(result[k]['los'])
        total = result[k]['los'] + result[k]['rec']
        data_ndvr[x].append(100.0*result[k]['los']/total)
...
for filename in args.pathvector:
    ...
    for k in result:
        ...
        if x not in data_pathvector:
            data_pathvector[x] = []
        #data_pathvector[x].append(result[k]['los'])
        total = result[k]['los'] + result[k]['rec']
        data_pathvector[x].append(100.0*result[k]['los']/total)
