...
void update_curtime() {
  ...
  std::strftime(curtime, sizeof(curtime), "%Y-%m-%d,%H:%M:%S",
                std::gmtime(&now));
}

#define NS_LOG(level, msg)                                                     \
  update_curtime();                                                            \
  std::cout << curtime << " [" << level << "] " << __func__ << "() " << msg    \
            << std::endl;
#define NS_LOG_DEBUG(msg) NS_LOG("DEBUG", msg)
#define NS_LOG_INFO(msg) NS_LOG("INFO", msg)
#define NS_LOG_WARN(msg) NS_LOG("WARN", msg)
#define NS_LOG_ERROR(msg) NS_LOG("ERROR", msg)
...
Ndvr::Ndvr(const ndn::security::SigningInfo &signingInfo, Name network,
           Name routerName, std::vector<std::string> &npv,
           std::vector<std::string> &faces,
           std::vector<std::string> &monitorFaces, std::string validationConfig)
    : m_signingInfo(signingInfo), m_scheduler(m_face.getIoService()),
      m_validator(m_face), m_seq(0),
      m_rand_nonce(0, std::numeric_limits<int>::max()),
      m_rand_backoff(1, 19999), m_network(network), m_routerName(routerName),
      m_listenFaces(faces), m_facesToBeMonitored(monitorFaces),
      m_routingTable(m_face, m_keyChain), m_helloIntervalIni(1),
      m_helloIntervalCur(1), m_helloIntervalMax(5)
      //, m_helloIntervalMax(60)
      ,
      m_localRTInterval(1), m_localRTTimeout(1), m_rengine(rdevice_()),
      m_pivot(m_neighMap.end()), m_faceMonitor(m_face) {
  buildRouterPrefix();

  try {
    m_validator.load(validationConfig);
  } catch (const std::exception &e) {
    throw Error("Failed to load validation rules file=" + validationConfig +
                " Error=" + e.what());
  }

  for (std::vector<std::string>::iterator it = npv.begin(); it != npv.end();
       ++it) {
...
void Ndvr::Start() {
  m_face.setInterestFilter(
      kNdvrHelloPrefix, std::bind(&Ndvr::processInterest, this, _2),
  ...
  m_face.setInterestFilter(
      routerDvInfoPrefix, std::bind(&Ndvr::processInterest, this, _2),
      [this](const Name &, const std::string &reason) {
        throw Error("Failed to register sync interest prefix: " + reason);
      });
  ...
  m_face.setInterestFilter(
      routerKey, std::bind(&Ndvr::OnKeyInterest, this, _2),
      [this](const Name &, const std::string &reason) {
        throw Error("Failed to register sync interest prefix: " + reason);
      });
  ...
  m_faceMonitor.onNotification.connect(
      std::bind(&Ndvr::onFaceEventNotification, this, _1));
  m_faceMonitor.start();
...
void Ndvr::registerNeighborPrefix(NeighborEntry &neighbor, uint64_t oldFaceId,
                                  uint64_t newFaceId) {
  // using namespace ns3;
...
void Ndvr::registerPrefixes() {
  ...
  //  using namespace ns3::ndn;
  //
  //  int32_t metric = 0; // should it be 0 or
  //  std::numeric_limits<int32_t>::max() ?? Ptr<Node> thisNode =
  //  NodeList::GetNode(Simulator::GetContext()); NS_LOG_DEBUG("THIS node is: "
  //  << thisNode->GetId());
  //
  //  for (uint32_t deviceId = 0; deviceId < thisNode->GetNDevices();
  //  deviceId++) {
  //    Ptr<NetDevice> device = thisNode->GetDevice(deviceId);
  //    Ptr<L3Protocol> ndn = thisNode->GetObject<L3Protocol>();
  //    NS_ASSERT_MSG(ndn != nullptr, "Ndn stack should be installed on the
  //    node");
  //
  //    auto face = ndn->getFaceByNetDevice(device);
  //    NS_ASSERT_MSG(face != nullptr, "There is no face associated with the
  //    net-device");
  //
  //    NS_LOG_DEBUG("FibHelper::AddRoute prefix=" << kNdvrPrefix << " via
  //    faceId=" << face->getId()); FibHelper::AddRoute(thisNode,
  //    kNdvrHelloPrefix, face, metric); FibHelper::AddRoute(thisNode,
  //    kNdvrDvInfoPrefix, face, metric);
  //
  //    auto addr = device->GetAddress();
  //    if (m_enableUnicastFaces && Mac48Address::IsMatchingType(addr)) {
  //      m_macaddr =
  //      boost::lexical_cast<std::string>(Mac48Address::ConvertFrom(addr));
...
void Ndvr::onFaceEventNotification(
  ...
  case ndn::nfd::FACE_EVENT_CREATED: {
    ...
    uint64_t faceId = faceEventNotification.getFaceId();
    auto foundFaceUri = std::find(m_facesToBeMonitored.begin(),
                                  m_facesToBeMonitored.end(), faceUri);
    if (foundFaceUri != m_facesToBeMonitored.end() && faceId != 0) {
      NS_LOG_DEBUG("Face creation event matches facesMonitor: "
                   << faceUri
                   << ". New Face ID: " << faceEventNotification.getFaceId()
                   << ". Registering NDVR prefixes.");
      m_routingTable.registerPrefix(kNdvrHelloPrefix.toUri(), faceId, 0);
      m_routingTable.registerPrefix(kNdvrDvInfoPrefix.toUri(), faceId, 0);
    }
    break;
...
std::string Ndvr::GetNeighborToken() {
  std::string res;
...
void Ndvr::SendHelloInterest() {
  /* check whether exists a scheduled SendHelloInterest son. If that is so,
   * skip this call */
  auto diff = time::duration_cast<time::seconds>(m_nextHelloTime -
  ...
  if (!params.empty()) {
    interest.setApplicationParameters(make_span(
        reinterpret_cast<const uint8_t *>(params.c_str()), params.size()));
  }
  ...
  m_nextHelloTime =
      time::steady_clock::now() + time::seconds(m_helloIntervalCur);
...
void Ndvr::RescheduleNeighRemoval(NeighborEntry &neighbor) {
  ...
  neighbor.removal_event = m_scheduler.schedule(
      neighbor.GetHelloTimeout(),
      // TODO: confirm the timeout is because the neighbor is no longer
      // reachable
...
void Ndvr::RemoveNeighbor(const std::string neigh) {
  ...
  // remove all routes whose next-hop is this neighbor (instead of remove, we
  // increase the cost)
  ...
  if (has_changed) {
    ...
    //  /* schedule a immediate ehlo message to notify neighbors about a new
    //  DvInfo */ ResetHelloInterval();
    SendHelloInterest();
  }
  // TODO: list my RIB
  NS_LOG_DEBUG("m_routingTable (one rib-entry per line)");
  for (auto it = m_routingTable.begin(); it != m_routingTable.end(); ++it) {
    NS_LOG_DEBUG("rib-entry: " << it->first << " seq=" << it->second.GetSeqNum()
                               << " nexhops={" << it->second.getNextHopsStr()
                               << "} bestnexthop="
                               << it->second.GetLearnedFrom());
  }
}

void Ndvr::SchedDvInfoInterest(NeighborEntry &neighbor, bool wait,
                               uint32_t retx) {
  ...
  NS_LOG_INFO("SchedDvInfoInterest name=" << n << " rand=" << r << "
  ...
  */
  ...
  NS_LOG_INFO("SchedDvInfoInterest name=" << n << " wait=" << wait
                                          << " backoffTime=" << backoffTime);

  dvinfointerest_event[n] =
      m_scheduler.schedule(time::microseconds(backoffTime),
                           [this, n, retx] { SendDvInfoInterest(n, retx); });
}

void Ndvr::SendDvInfoInterest(const std::string &neighbor_name, uint32_t retx) {
  ...
  NS_LOG_INFO("Sending DV-Info Interest retx=" << retx << " to neighbor="
                                               << neighbor_name);
...
uint64_t Ndvr::ExtractIncomingFace(const ndn::Interest &interest) {
   ...
   * NDNLPv2 says "Incoming face indication feature allows the forwarder to
   * inform local applications about the face on which a packet is received."
   * and also warns "application MUST be prepared to receive a packet without
   * IncomingFaceId field". From our tests, only internal faces seems to not
   * initialize it and this can even be used to filter out our own interests.
   * See more:
   *  - FibManager::setFaceForSelfRegistration
   * (ndnSIM/NFD/daemon/mgmt/fib-manager.cpp)
   *  -
  ...
  shared_ptr<lp::IncomingFaceIdTag> incomingFaceIdTag =
      interest.getTag<lp::IncomingFaceIdTag>();
...
uint64_t Ndvr::ExtractIncomingFace(const ndn::Data &data) {
  shared_ptr<lp::IncomingFaceIdTag> incomingFaceIdTag =
      data.getTag<lp::IncomingFaceIdTag>();
  if (incomingFaceIdTag == nullptr) {
    return 0;
  }
  return *incomingFaceIdTag;
...
void Ndvr::IncreaseHelloInterval() {
  ...
  if (increasehellointerval_event)
    return;
  increasehellointerval_event = m_scheduler.schedule(time::seconds(1), [this] {
    m_helloIntervalCur = (2 * m_helloIntervalCur > m_helloIntervalMax)
                             ? m_helloIntervalMax
                             : 2 * m_helloIntervalCur;
  });
}
...
void Ndvr::OnHelloInterest(const ndn::Interest &interest, uint64_t inFaceId) {
  ...
  if (interest.hasApplicationParameters() &&
      interest.getApplicationParameters().value_size() > 0) {
    std::string s;
    s.assign((char *)interest.getApplicationParameters().value(),
  ...
  if (neigh == m_neighMap.end()) {
    ...
    if (m_enableUnicastFaces && !neigh_mac.empty()) {
      ...
      if (neighFaceId_it == m_neighToFaceId.end()) {
        ...
        NS_LOG_INFO("NEW-Neighbor_FaceId == " << neighFaceId
                                              << " mac = " << neigh_mac);
        m_neighToFaceId.emplace(neighPrefix, neighFaceId);
      } else {
        neighFaceId = neighFaceId_it->second;
        NS_LOG_INFO("EXISTING-Neighbor_FaceId == " << neighFaceId
                                                   << " mac = " << neigh_mac);
      }
    }
    if (neighFaceId == 0)
    ...
    neigh = m_neighMap
                .emplace(neighPrefix,
                         NeighborEntry(neighPrefix, neighFaceId, version))
                .first;
    ...
    if (neigh->second.GetFaceId() != inFaceId) {
      /* Issue #2: TODO: We need to be careful about this because since we are
       * using default multicast forward strategy, mean that one node can
       * forward ndvr messages from other nodes, so the interest might be
       * received from the direct face or from the intermediate forwarder face!
       * Not necessarily means the node moved! */
      // NS_LOG_INFO("Neighbor moved from faceId=" << neigh->second.GetFaceId()
      // << " to faceId=" << inFaceId << " neigh=" << neighPrefix);
      // registerNeighborPrefix(neigh->second, neigh->second.GetFaceId(),
      // inFaceId);
    }
  ...
  // && numPrefixes >= m_routingTable.size()) {
  if (numPrefixes > 0 && (newNeigh || version > neigh->second.GetVersion())) {
    ...
    NS_LOG_INFO("NDVR converged - Skipped DvInfoInterest numPrefixes="
                ...
                << " saved_version=" << neigh->second.GetVersion());
  }
}
...
void Ndvr::OnDvInfoInterest(const ndn::Interest &interest) {
  ...
  std::string routerPrefix =
      ExtractRouterPrefix(interest.getName(), kNdvrDvInfoPrefix);
  if (routerPrefix != m_routerPrefix) {
    // NS_LOG_INFO("Interest is not to me, ignoring.. received_name=" <<
    // routerPrefix << " my_name=" << m_routerPrefix);
    return;
  }
  ...
  if (replydvinfo_event)
    return;
  replydvinfo_event =
      m_scheduler.schedule(time::milliseconds(replydvinfo_dist(m_rengine)),
                           [this, interest] { ReplyDvInfoInterest(interest); });
}
...
void Ndvr::ReplyDvInfoInterest(const ndn::Interest &interest) {
  ...
  data->setContent(
      make_span(reinterpret_cast<const uint8_t *>(dvinfo_str.c_str()),
                dvinfo_str.size()));
  // Sign and send
...
void Ndvr::OnDvInfoTimedOut(const ndn::Interest &interest, uint32_t retx) {
  // TODO: Apply the same logic as in HelloProtocol::processInterestTimedOut
  // (~/mini-ndn/ndn-src/NLSR/src/hello-protocol.cpp)
  ...
  /* Sanity checks */
  std::string neighPrefix =
      ExtractRouterPrefix(interest.getName(), kNdvrDvInfoPrefix);
  if (!isValidRouter(interest.getName(), kNdvrDvInfoPrefix) ||
      neighPrefix == m_routerPrefix) {
    return;
  }
  ...
  /* is it worth to send a DvInfo now instead of waiting the next hello/dvinfo
   * exchange cycle? */
...
void Ndvr::OnDvInfoContent(const ndn::Interest &interest,
  ...
  if (data.getSignatureInfo().hasKeyLocator()) {
    NS_LOG_DEBUG("Data signed with: "
                 << data.getSignatureInfo().getKeyLocator().getName());
  }

  // Validating data
  m_validator.validate(
      data, std::bind(&Ndvr::OnValidatedDvInfo, this, _1),
...
void Ndvr::OnValidatedDvInfo(const ndn::Data &data) {
  ...
  //   NS_LOG_INFO("DV-Info from neighbor prefix=" << entry.prefix() << "
...
void Ndvr::OnDvInfoValidationFailed(
    const ndn::Data &data, const ndn::security::v2::ValidationError &ve) {
  NS_LOG_DEBUG("Not validated data: " << data.getName()
                                      << ". The failure info: " << ve);
}
...
void Ndvr::processDvInfoFromNeighbor(NeighborEntry &neighbor,
  ...
  for (auto entry : otherRT) {
    ...
    NS_LOG_INFO("===>> prefix=" << neigh_prefix << " seqNum=" << neigh_seq
                                << " recvCost=" << neigh_cost << " learnedFrom="
                                << entry.second.GetLearnedFrom());
    ...
    if (localRE == nullptr) {
      ...
      NS_LOG_INFO("======>> New prefix! Just insert it "
                  << neigh_prefix << " via " << neighbor.GetFaceId());
    ...
    //   if (localRE->GetOriginator() == m_routerPrefix && neigh_seq >
    //   localRE->GetSeqNum()) {
    //     localRE->IncSeqNum(2);
    //     has_changed = true;
    //   }
    ...
    if (!localRE->isNextHop(neighbor.GetFaceId())) {
      ...
      NS_LOG_INFO("======>> New neighbor! Just insert it "
                  << neigh_prefix << " via " << neighbor.GetFaceId());
    ...
    if (isInfinityCost(neigh_cost)) {
      if (neigh_seq > localRE->GetSeqNum()) {
        NS_LOG_INFO("======>> New SeqNum infinity cost, update! local_seqNum="
                    << localRE->GetSeqNum() << " neigh_seqNum=" << neigh_seq);
        localRE->SetSeqNum(neigh_seq);
      }

      NS_LOG_INFO("======>> Infinity cost! Remove nextHop for name prefix"
                  << neigh_prefix << " nextHop=" << neighbor.GetFaceId());
    ...
    if (neigh_seq > localRE->GetSeqNum()) {
      ...
      if (entry.second.GetLearnedFrom() ==
          m_routerPrefix) { //&& isInfinityCost(neigh_sec_cost)) {
        NS_LOG_INFO("======>> New SeqNum and learned only from ourself! Remove "
                    "nextHop for name prefix "
                    << neigh_prefix << " nextHop=" << neighbor.GetFaceId()
                    << " local_seqNum=" << localRE->GetSeqNum()
                    << " neigh_seqNum" << neigh_seq);
      ...
      NS_LOG_INFO("======>> New SeqNum, update name prefix! local_seqNum="
                  << localRE->GetSeqNum() << " neigh_seqNum=" << neigh_seq
                  << " local_cost=" << localRE->GetCost(neighbor.GetFaceId())
      ...
      ////   - Recv_Cost != Local_cost: wait SettlingTime, then update
      /// Local_Cost / Local_SeqNum
      // if (localRE->GetCost() == neigh_cost) {
      //   NS_LOG_INFO("======>> New SeqNum same cost, update name prefix!
      //   local_seqNum=" << localRE->GetSeqNum() << " neigh_seqNum=" <<
      //   neigh_seq); localRE->SetSeqNum(neigh_seq); has_changed = true; if
      //   (!localRE->isNextHop(neighbor.GetFaceId())) {
      //     NS_LOG_INFO("======>> New SeqNum same cost from other next-hop,
      //     update!"); m_routingTable.UpdateRoute(*localRE,
      //     neighbor.GetFaceId());
      //   }
      // } else if (neigh_cost < localRE->GetCost()) {
      //   NS_LOG_INFO("======>> New SeqNum diff lower cost, update name prefix!
      //   local_seqNum=" << localRE->GetSeqNum() << " neigh_seqNum=" <<
      //   neigh_seq << " local_cost=" << localRE->GetCost() << " neigh_cost="
      //   << neigh_cost);
      ...
      //     NS_LOG_INFO("======>> New SeqNum higher cost from same next-hop or
      //     even seq, update name prefix! local_seqNum=" <<
      //     localRE->GetSeqNum() << " neigh_seqNum=" << neigh_seq);
      //     localRE->SetCost(neigh_cost);
      //     localRE->SetSeqNum(neigh_seq);
      //     m_routingTable.UpdateRoute(*localRE, neighbor.GetFaceId());
      //     has_changed = true;
      //   } else {
      //     NS_LOG_INFO("======>> New SeqNum higher cost from other next-hop
      //     and odd seqNum, just increase seqNum! local_seqNum=" <<
      //     localRE->GetSeqNum() << " neigh_seqNum=" << neigh_seq);
      //     localRE->SetSeqNum(neigh_seq);
      //     has_changed = true;
      //   }
      // }
    } else if (neigh_seq == localRE->GetSeqNum() &&
               neigh_cost != localRE->GetCost(neighbor.GetFaceId())) {
      NS_LOG_INFO(
          "======>> Equal SeqNum but diff cost, update name prefix! local_cost="
          << localRE->GetCost(neighbor.GetFaceId())
          << " neigh_cost=" << neigh_cost);
      /* Cost change will be handle by periodic updates */
      ...
      m_routingTable.UpsertNextHop(*localRE, neighbor.GetFaceId(), neigh_cost,
                                   neighbor.GetName());
      has_changed = true;
      //} else if (neigh_seq == localRE->GetSeqNum() && neigh_cost ==
      // localRE->GetCost()) {
      //  NS_LOG_INFO("======>> Equal SeqNum and Equal Cost, local_cost=" <<
      //  localRE->GetCost());
      //} else if (neigh_seq == localRE->GetSeqNum() && neigh_cost >
      // localRE->GetCost()) {
      //  //NS_LOG_INFO("======>> Equal SeqNum and (Equal or Worst Cost),
      //  however learn name prefix! local_cost=" << localRE->GetCost());
      //  // TODO: save this new prefix as well to multipath
    } else {
      /* Recv_SeqNum < Local_SeqNu: discard/next, we already have a most recent
       * update */
      continue;
    }
  }
...
bool Ndvr::isInfinityCost(uint32_t cost) {
  return cost == std::numeric_limits<uint32_t>::max();
}
...
uint64_t Ndvr::CreateUnicastFace(std::string mac) {
  ...
  //  ns3::Ptr<ns3::ndn::L3Protocol> ndn =
  //  thisNode->GetObject<ns3::ndn::L3Protocol>(); std::string myUrl =
  //  "netdev://[" +
  //  boost::lexical_cast<std::string>(ns3::Mac48Address::ConvertFrom(netDevice->GetAddress()))
  //  + "]";
  ...
  //
  //  auto linkService =
  //  std::make_unique<::nfd::face::GenericLinkService>(opts);
  //
  //
  //  auto transport =
  //  std::make_unique<ns3::ndn::UnicastNetDeviceTransport>(thisNode, netDevice,
  //                                                   myUrl,
  //                                                   //neighUri);
  //                                                   mac);
  //  auto face = std::make_shared<::nfd::face::Face>(std::move(linkService),
  //  std::move(transport)); face->setMetric(1);
  //
  //  ndn->addFace(face);
  //
  //  return face->getId();
